package app.oribu.data.repository

import app.oribu.data.db.dao.BookQuoteDao
import app.oribu.data.db.dao.GamePlaythroughDao
import app.oribu.data.db.dao.MangaReviewDao
import app.oribu.data.db.dao.MediaItemDao
import app.oribu.data.db.dao.MovieListDao
import app.oribu.data.db.dao.SeriesEpisodeDao
import app.oribu.data.db.entity.BookQuoteEntity
import app.oribu.data.db.entity.GamePlaythroughEntity
import app.oribu.data.db.entity.MangaReviewEntity
import app.oribu.data.db.entity.MediaItemEntity
...
import app.oribu.model.BookQuote
...
class MediaRepository(
    ...
    suspend fun getAll(): List<MediaItem> = itemDao.getAll().map { it.toDomain() }

    suspend fun getByType(type: MediaType): List<MediaItem> = itemDao.getByType(type.dbValue).map { it.toDomain() }

    suspend fun getByTypes(types: List<MediaType>): List<MediaItem> = itemDao.getByTypes(types.map { it.dbValue }).map { it.toDomain() }

    suspend fun getById(id: Int): MediaItem? = itemDao.getById(id)?.toDomain()

    fun watchByType(type: MediaType): Flow<List<MediaItem>> = itemDao.watchByType(type.dbValue).map { list -> list.map { it.toDomain() } }
    ...
    fun watchAll(): Flow<List<MediaItem>> = itemDao.watchAll().map { list -> list.map { it.toDomain() } }

    suspend fun save(item: MediaItem): Int = itemDao.insert(MediaItemEntity.fromDomain(item)).toInt()

    suspend fun update(item: MediaItem) = itemDao.update(MediaItemEntity.fromDomain(item))

    suspend fun delete(id: Int) = itemDao.delete(id)
    ...
    suspend fun updateList(
        ...
        name: String,
        description: String?,
    ) = listDao.update(id, name, description)

    suspend fun renameList(
        id: Int,
        newName: String,
    ) = listDao.rename(id, newName)
    ...
    suspend fun removeItemFromAllLists(mediaItemId: Int) = listDao.removeItemFromAll(mediaItemId)
    ...
    suspend fun unmarkEpisode(
        mediaItemId: Int,
        season: Int,
        episode: Int,
    ) = episodeDao.unmark(mediaItemId, season, episode)

    suspend fun deleteSeriesEpisodes(mediaItemId: Int) = episodeDao.deleteBySeries(mediaItemId)

    // ── Manga review history ──────────────────────────────────────────────────

    suspend fun mangaReviewHistory(mediaItemId: Int): List<MangaReview> = mangaReviewDao.getByItem(mediaItemId).map { it.toDomain() }

    suspend fun archiveMangaReview(
        mediaItemId: Int,
        rating: Double?,
        reviewTitle: String?,
        reviewText: String?,
    ...
    suspend fun savePlaythrough(
        mediaItemId: Int,
        playthrough: GamePlaythrough,
    ) {
        val entity =
            GamePlaythroughEntity(
                id = playthrough.id,
                mediaItemId = mediaItemId,
