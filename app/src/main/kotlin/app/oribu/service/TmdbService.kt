...
class TmdbService(
    ...
    fun getSeasonEpisodes(
        ...
        return (get(url)["episodes"] as? List<*>)
            ?.filterIsInstance<Map<String, Any?>>()
            ?.map { TmdbEpisode.fromJson(it) } ?: emptyList()
    }
    ...
    private fun streamingProviderId(name: String): Int? =
        mapOf(
            "netflix" to 8,
            "prime video" to 119,
            "disney+" to 337,
    ...
    private fun mapMovie(r: Map<String, Any?>) =
        ApiSearchResult(
    ...
    private fun mapSeries(r: Map<String, Any?>) =
        ApiSearchResult(
            externalId = (r["id"] as? Double)?.toInt()?.toString() ?: r["id"]?.toString() ?: "",
    ...
    private fun parseDate(raw: String?): Date? {
        ...
        return try {
            java.text.SimpleDateFormat("yyyy-MM-dd", java.util.Locale.US).parse(raw)
        } catch (_: Exception) {
            null
        }
    }
    ...
    companion object {
        ...
        private fun translateRole(job: String) =
            mapOf(
                ...
                "Writer" to "Roteiro",
                "Producer" to "Produção",
                "Executive Producer" to "Produção Executiva",
                "Director of Photography" to "Fotografia",
                "Original Music Composer" to "Trilha Sonora",
                "Editor" to "Edição",
                "Creator" to "Criação",
...
data class TmdbSeason(
    ...
    companion object {
        fun fromJson(j: Map<String, Any?>): TmdbSeason {
            val date =
                try {
                    java.text.SimpleDateFormat("yyyy-MM-dd", java.util.Locale.US).parse(j["air_date"] as? String ?: "")
                } catch (
                    _: Exception,
                ) {
                    null
...
data class TmdbMovieDetails(
    ...
    val related: List<TmdbRelatedMovie> = emptyList(),
) {
    ...
    companion object {
        fun fromJson(j: Map<String, Any?>): TmdbMovieDetails {
            ...
            val date =
                try {
                    java.text.SimpleDateFormat("yyyy-MM-dd", java.util.Locale.US).parse(j["release_date"] as? String ?: "")
                } catch (
                    _: Exception,
                ) {
                    null
...
data class TmdbSeriesDetails(
    ...
    val crew: List<TmdbPerson> = emptyList(),
    val providers: List<TmdbProvider> = emptyList(),
    val related: List<TmdbRelatedSeries> = emptyList(),
) {
    val posterUrl get() = posterPath?.let { "https://image.tmdb.org/t/p/w500$it" }
    val backdropUrl get() = backdropPath?.let { "https://image.tmdb.org/t/p/w780$it" }
        ...
        fun fromJson(j: Map<String, Any?>): TmdbSeriesDetails {
            ...
            val crewRaw =
                (credits?.get("crew") as? List<*>)
                    ...
                    ?: emptyList()
            val creators =
                (j["created_by"] as? List<*>)?.filterIsInstance<Map<String, Any?>>()?.map {
                    TmdbPerson(
            ...
            val first =
                try {
                    fmt.parse(j["first_air_date"] as? String ?: "")
                } catch (_: Exception) {
                    null
                }
            val last =
                try {
                    fmt.parse(j["last_air_date"] as? String ?: "")
                } catch (_: Exception) {
                    null
                }
            val seasons =
