...
class SteamService(
    ...
    /**
     * `get()` não lança em respostas não-2xx e a Steam Web API devolve um `response` vazio (sem
     * "players") tanto para chave inválida quanto para SteamID inválido — trata os dois casos
     * como falha de validação.
     */
    fun testConnection() {
    ...
    fun getLibrary(includeUnplayed: Boolean = true): List<SteamGame> {
        ...
        val games =
            ((get(url)["response"] as? Map<*, *>)?.get("games") as? List<*>)
                ?.filterIsInstance<Map<String, Any?>>() ?: return emptyList()
        return games
            .map { SteamGame.fromJson(it) }
    ...
    fun getAchievements(appId: String): List<SteamAchievement> {
        ...
        return (player["achievements"] as? List<*>)
