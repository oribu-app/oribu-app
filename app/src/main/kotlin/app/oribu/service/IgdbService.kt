...
class IgdbService(
    ...
    private val mainPlatforms =
        setOf(6, 3, 14, 48, 167, 9, 8, 38, 46, 49, 169, 12, 11, 130, 137, 390, 508, 41, 5, 20, 37, 21, 4, 19, 18, 24, 33, 22)
    ...
    private val platformDisplayNames =
        mapOf(
    ...
    private val invalidTitlePattern =
        Regex(
            """\b(fan[\s\-]?game|fangame|fan[\s\-]?made|unofficial|rom\s?hack|romhack|demo|tech\s+demo|open\s+beta|closed\s+beta|beta|trial|preview|network\s+test|kaizo|online\s+mode)\b|\d+\.\d+\.\d+""",
    ...
    private val baseFields =
        "name, cover.url, first_release_date, genres.name, category, " +
    ...
    fun getRelatedGames(igdbId: Int): IgdbRelatedGames {
        val body =
            "fields dlcs.name, dlcs.cover.url, expansions.name, expansions.cover.url, " +
    ...
    fun getDetails(name: String): Map<String, Any?>? {
        ...
        val platforms =
            (r["platforms"] as? List<*>)?.filterIsInstance<Map<String, Any?>>()?.map { it["name"] as? String ?: "" } ?: emptyList()
        val releaseDate =
            (r["first_release_date"] as? Double)?.toLong()?.let {
                val dt = Date(it * 1000)
                val cal =
                    java.util.Calendar
                        .getInstance()
            ...
            tokenFile(context).writeText(
                Gson().toJson(
                    mapOf(
                        ...
                        "obtained_at" to token.obtainedAt.time,
