...
import app.oribu.data.db.dao.GameCacheDao
import app.oribu.data.db.entity.GameCacheEntity
...
class GameCacheService(
    ...
    suspend fun enrichWithIgdb(name: String): GameCacheEntity? {
        ...
        return try {
            ...
            val releaseDateStr =
                existing?.releaseDate ?: igdbResult.releaseDate?.let {
                    dateFmt.format(it)
                }
    ...
    private fun extractConcept(
        ...
        setIn
            ...
            ?.groupValues
            ?.get(1)
            ?.let { cleanFragment(it).takeIf { f -> f.length >= 5 }?.let { return "set in $it" } }
        ...
        val wherePlayers =
            Regex("""\bwhere\s+players?\s+(?:can|must|will|are|may|have\s+to\s+)?(\w[\w\s]{5,55})""", RegexOption.IGNORE_CASE)
        wherePlayers.find(summary)?.groupValues?.get(1)?.let {
            cleanFragment(it)
                .takeIf { f ->
                    f.length >= 5
                }?.let { return "where players $it" }
        }
        ...
        about
            ...
            ?.groupValues
            ?.get(1)
            ?.let { cleanFragment(it).takeIf { f -> f.length >= 5 }?.let { return "about $it" } }
